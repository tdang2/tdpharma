class Api::V1::ReceiptsController < ApplicationController
  ...
  def update
    raise 'Author is required' if params[:receipt][:transactions_attributes].any?{|t| t[:user_id].blank?}
    receipt = @store.receipts.find(params[:id])
  ...
  def receipt_params
    params.require(:receipt).permit(:receipt_type, :total, :store_id,
                                    ...
                                    purchase_transactions_atttributes: [:id, :amount, :new_total, :delivery_time, :due_date, :performed,
                                                                        :status, :transaction_type, :total_price, :notes,
