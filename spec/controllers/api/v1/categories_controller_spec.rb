...
RSpec.describe Api::V1::CategoriesController, type: :controller do
  ...
  describe 'POST create' do
    it 'create and return all store categories' do
      sign_in u1
      post :create, token: u1.authentication_token, category: category_params, format: :json
      expect(response.status).to eq 200
      expect(JSON.parse(response.body)['authentication_token']).to eq u1.authentication_token
  ...
  describe 'PATCH update' do
    it 'update and return all store categories' do
      sign_in u1
      patch :update, id: @c5.id, token: u1.authentication_token, category: category_params, format: :json
  ...
  describe 'GET show' do
    it 'return category and its children' do
      sign_in u1
      get :show, id: @c3.id, token: u1.authentication_token, format: :json
      expect(response.status).to eq 200
      expect(JSON.parse(response.body)['authentication_token']).to eq u1.authentication_token
  ...
  describe 'DELETE destroy' do
    it 'destroy and return store categories' do
      sign_in u1
      delete :destroy, id: @c1.id, token: u1.authentication_token, format: :json
      expect(response.status).to eq 200
      expect(JSON.parse(response.body)['authentication_token']).to eq u1.authentication_token
