...
RSpec.describe Api::V1::InventoryItemsController, type: :controller do
  ...
  describe 'GET index' do
    ...
    describe 'with interested  item id' do
      it 'return page of items' do
        get :index, inventory_id: item1.id, access_token: token.token, format: :json
        expect(response.status).to eq 200
  ...
  describe 'GET show' do
    it 'should return the inventory items' do
      ...
      expect(item1.med_batches.map(&:id)).to include JSON.parse(response.body)['available_batches'][0]['id']
      expect(Float(JSON.parse(response.body)['available_batches'][0]['total_units'])).to be > 0
      expect(JSON.parse(response.body)['available_batches'].all? {|t| t['status'] == 'active'}).to eq true
    end
  ...
  describe 'PATCH update' do
    it 'turn inventory to inactive' do
      patch :update, id: item1.id, format: :json, inventory_item: {status: 'inactive'}, access_token: token.token
      expect(response.status).to eq 200
      expect(JSON.parse(response.body)['id']).to eq item1.id
    ...
    it 'create sale price for inventory' do
      patch :update, id: item4.id, format: :json, inventory_item: inventory_item_new_price_params, access_token: token.token
      expect(response.status).to eq 200
      expect(JSON.parse(response.body)['id']).to eq item4.id
      expect(JSON.parse(response.body)['sale_price']['amount']).to eq 200
      expect(JSON.parse(response.body)['available_batches'].all? {|t| t['status'] == 'active'}).to eq true
    end
    it 'update sale price for inventory' do
      patch :update, id: item1.id, inventory_item: inventory_item_price_params, format: :json, access_token: token.token
      expect(response.status).to eq 200
      expect(JSON.parse(response.body)['id']).to eq item1.id
      expect(JSON.parse(response.body)['sale_price']['amount']).to eq 200
      expect(JSON.parse(response.body)['available_batches'].all? {|t| t['status'] == 'active'}).to eq true
