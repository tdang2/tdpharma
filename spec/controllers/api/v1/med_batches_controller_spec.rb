...
RSpec.describe Api::V1::MedBatchesController, type: :controller do
  ...
  describe 'GET index' do
    it 'get store med batches' do
      ...
      expect(JSON.parse(response.body).map{|i| i['inventory_item']['id']}).to include item1.id
      expect(JSON.parse(response.body).map{|i| i['medicine']['name']}).to include item2.itemable.name
      expect(JSON.parse(response.body).map{|i| i['category']['name']}).to include item3.category.name
    end
    ...
    it 'get batches through barcode' do
      ...
      expect(JSON.parse(response.body)[0]['medicine']['name']).to eq item1.itemable.name
      expect(JSON.parse(response.body)[0]['category']['name']).to eq item1.category.name
    end

    it 'get batches with barcode regardless empty or available batch' do
      item2.med_batches.each {|b| b.update!(total_units: 0, number_pkg: 0)}
      get :index, barcode: item2.med_batches.first.barcode, format: :json, access_token: token.token
      expect(JSON.parse(response.body).count).to eq 1
      expect(JSON.parse(response.body)[0]['inventory_item']['id']).to eq item2.id
      expect(JSON.parse(response.body)[0]['medicine']['name']).to eq item2.itemable.name
      expect(JSON.parse(response.body)[0]['category']['name']).to eq item2.category.name
    end
  end
