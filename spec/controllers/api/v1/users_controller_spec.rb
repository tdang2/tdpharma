...
RSpec.describe Api::V1::UsersController, type: :controller do
  ...
  describe 'PATCH update' do
    ...
    it 'update user info' do
      sign_in u1
      patch :update, id: u1.id, email: u1.email, token: u1.authentication_token, user: users_patch_params, format: :json
      expect(response.status).to eq 200
    ...
    it 'update user role as manager' do
      sign_in u3
      patch :update, id: u1.id, email: u3.email, token: u3.authentication_token, role_ids: [Role.first.id, role1.id], format: :json
      expect(response.status).to eq 200
    ...
    it 'can not update user as employee' do
      sign_in u1
      patch :update, id: u1.id, email: u1.email, token: u1.authentication_token, user: users_patch_params, role_ids: [Role.first.id, role1.id], format: :json
  ...
  describe 'GET show' do
    ...
    it 'get user info' do
      sign_in u1
      get :show, id: u1.id, email: u1.email, token: u1.authentication_token, format: :json
      expect(response.status).to eq 200
    end
  ...
  describe 'GET destroy' do
    ...
    it 'can not destroy itself as employee' do
      sign_in u1
      expect {delete :destroy, id: u1.id, email: u1.email, token: u1.authentication_token, format: :json} .to raise_error CanCan::AccessDenied
    end
    it 'destroy itself as manager' do
      sign_in u3
      delete :destroy, id: u3.id, email: u3.email, token: u3.authentication_token, format: :json
      expect(response.status).to eq 200
    ...
    it 'destroy employee as manager' do
      sign_in u3
      delete :destroy, id: u1.id, email: u3.email, token: u3.authentication_token, format: :json
      expect(response.status).to eq 200
