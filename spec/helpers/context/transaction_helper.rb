RSpec.shared_context 'transaction params' do
  let(:purchase_transaction_edit_params) do
    {
      ...
      paid: true,
      performed: true,
      user_id: u1.id,    # User id who make the purchase edit
      inventory_item_id: item1.id,    # id of inventory item. only need to provide when change purchase inventory item
      total_price: 250,           # total for the purchase of this inventory item batch
      store_id: s.id,            # No need for client side with logged in info
  ...
  let(:sale_transaction_edit_params) do
    {
        ...
        paid: true,
        performed: true,
        user_id: u1.id,     # User id who made the sale edit
        inventory_item_id: item1.id,  # Item id must be provided and match batch id. No need to provide in real api request
  ...
  let(:sale_different_batch_edit_params) do
    {
        ...
        paid: false,
        performed: false,
        user_id: u2.id,    # must provide user id who perform the update
        inventory_item_id: item2.id, # new item id that will be sold instead of item 1
