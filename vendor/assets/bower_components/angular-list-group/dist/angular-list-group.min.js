(function (window, document) {
  ...
  angularListGroupFilters.filter('startsWith', [
    '$filter',
    function ($filter) {
  ...
  angularListGroupFilters.filter('eq', [
    '$filter',
    function ($filter) {
  ...
  angularListGroupFilters.filter('neq', [
    '$filter',
    function ($filter) {
      return function (str, text, ignoreCase) {
        var comparator = function (str, text) {
  ...
  angularListGroupFilters.filter('endsWith', [
    '$filter',
    function ($filter) {
      return function (str, text, ignoreCase) {
  ...
  angularListGroupFilters.filter('contains', [
    '$filter',
    function ($filter) {
  ...
  var ListGroupCtrl = [
      ...
      function ($scope, $attrs, $parse, $filter, $sce, $compile, $timeout) {
        ...
        ctrl.beforeSelectionChange = !$attrs.beforeSelectionChange ? defaultBeforeSelectionChange : $scope.beforeSelectionChange;
        ...
        ctrl.select = function (item) {
          if (!ctrl.isDisabled(item)) {
            var output = ctrl.beforeSelectionChange({ item: item });
            if (angular.isUndefined(output)) {
              throw new Error('\'beforeSelectionChange\' returned undefined as value! Check the binding or the returned value');
            }
            if (output) {
              if (angular.isFunction(output.then)) {
                output.then(function (returnedValue) {
                  if (returnedValue === true) {
                    ...
                    $timeout(function () {
                      ctrl.afterSelectionChange({ item: item });
                    }, 50, true);
                  }
                });
              } else if (output === true) {
        ...
        ctrl.resolveContextualClass = function (item) {
          ...
          if ($attrs.contextualClass) {
            ...
            if (angular.isFunction(fn)) {
              var val = fn($scope.$parent, { item: item });
              if (val) {
                clazz = val;
              }
            }
        ...
        var removeSelectedItemsListener = $scope.$watchCollection('ctrl.$$selectedItems', function (newValue, oldValue) {
  ...
  angularListGroupDirectives.directive('listGroup', [
    '$templateCache',
  ...
  angularListGroupDirectives.directive('listGroupItemContent', [
    '$compile',
    '$templateRequest',
        ...
        controller: function ($scope, $attrs, $parse) {
          ...
          ctrl.resolveLabel = function (item) {
            ...
            if ($scope.labelFn) {
              ...
              // Client Ctrl scope
              targetScope = targetScope.$parent;
              label = fn(targetScope, { item: item });
            } else if (item.label) {
              label = item.label;
      ...
      function ($scope, $attrs, $parse, $filter, $sce, $compile, $injector, $timeout) {
        ...
        ctrl.$$onDelete = function (item) {
          if ('onDelete' in $attrs) {
            var output = $scope.onDelete({ item: item });
            if (isPromise(output)) {
        ...
        ctrl.$$onEdit = function (item) {
          if ('onEdit' in $attrs) {
            var output = $scope.onEdit({ item: angular.copy(item) });
            if (isPromise(output)) {
        ...
        ctrl.isActionDisabled = function (item, action) {
          ...
          if (action.disabled) {
            returnedValue = action.disabled($scope.$parent, { item: item });
          }
